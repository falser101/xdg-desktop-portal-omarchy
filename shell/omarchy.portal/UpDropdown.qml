...
// Same visuals as qs.Ui.Dropdown, but the menu opens upward so a footer
// filter is not clipped by the FloatingWindow bottom edge.
Item {
  id: root
  ...
  Column {
    ...
    BorderSurface {
      id: trigger
      width: parent.width
      height: (root.showLabel && root.label !== "") ? root.rowHeight : parent.height
  ...
  Popup {
    ...
    implicitHeight: Math.min(
      root.options.length * root.popupRowHeight + Math.max(0, root.options.length - 1) * Style.spacing.labelGap + Style.spacing.xxs,
    ...
    onAboutToShow: {
      ...
      popup.y = pos.y - h - gap
      if (popup.y < gap)
        popup.y = pos.y + trigger.height + gap
    }
